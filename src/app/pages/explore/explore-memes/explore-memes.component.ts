...
@Component({
  ...
  styleUrl: './explore-memes.component.css',
})
export class ExploreMemesComponent implements OnInit {
  ...
  loadFirstPage(): void {
    ...
    this.rantService.searchRants('#memes', 1, this.pageSize).subscribe({
      next: (items) => {
      ...
      error: (err) => {
        console.error('Failed to load memes posts', err);
        this.error.set('Failed to load memes posts.');
        this.isLoading.set(false);
      },
    });
  }
  ...
  loadNextPage(): void {
    ...
    this.rantService.searchRants('#memes', nextPage, this.pageSize).subscribe({
      next: (items) => {
      ...
      error: (err) => {
        console.error('Failed to load more memes posts', err);
        this.isLoadingMore.set(false);
      },
    });
  }
